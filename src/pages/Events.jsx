...
function EventCard({ event, onRegister, isMobile }) {
  ...
  return (
    <div style={{ border: "1px solid #2d3f55", borderRadius: "14px", background: "#1e293b", boxShadow: "0 2px 10px rgba(0,0,0,0.06)", overflow: "hidden", display: "flex", flexDirection: "column" }}>
      <div style={{ padding: "18px 18px 0" }}>
        <div style={{ display: "inline-block", background: "rgba(47,141,70,0.15)", color: "#2f8d46", fontSize: "0.7rem", fontWeight: "700", padding: "3px 10px", borderRadius: "999px", marginBottom: "8px", textTransform: "uppercase", letterSpacing: "0.05em" }}>{event.category}</div>
        <div style={{ fontWeight: "700", fontSize: isMobile ? "0.97rem" : "1.05rem", marginBottom: "8px" }}>{event.title}</div>
        <div style={{ display: "flex", flexDirection: "column", gap: "4px", marginBottom: "10px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "7px", fontSize: "0.83rem", color: "#94a3b8" }}>
      ...
      <p style={{ color: "#94a3b8", fontSize: "0.87rem", lineHeight: "1.6", padding: "0 18px 12px" }}>{event.description}</p>
      <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", padding: "0 18px 14px" }}>
        {event.tags.map(t => <span key={t} style={{ background: "#263348", color: "#94a3b8", fontSize: "0.73rem", padding: "3px 9px", borderRadius: "6px", fontWeight: "500" }}>{t}</span>)}
      </div>
      <div style={{ padding: "12px 18px", borderTop: "1px solid #f0f0f0", display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: "auto" }}>
        <div style={{ fontSize: "0.8rem", color: "#64748b", display: "flex", alignItems: "center", gap: "7px" }}>
          <div style={{ background: "rgba(47,141,70,0.15)", borderRadius: "999px", height: "6px", width: "64px", overflow: "hidden" }}>
            <div style={{ height: "100%", borderRadius: "999px", background: pct >= 90 ? "#ef4444" : "#2f8d46", width: pct + "%" }} />
          </div>
          {event.registered}/{event.seats}
        </div>
        {isPast
          ? <button disabled style={{ background: "#263348", color: "#475569", border: "none", borderRadius: "7px", padding: "7px 16px", cursor: "not-allowed", fontSize: "0.85rem", fontWeight: "600" }}>Ended</button>
...
function CoordinatorAddEvent({ setEvents, isMobile }) {
  ...
  const select = { ...input, background: "#1e293b", cursor: "pointer" }
  ...
  if (!unlocked) return (
    <div>
      ...
      <div style={{ background: "#1e293b", border: "1px solid #fde68a", borderRadius: "12px", padding: isMobile ? "20px 16px" : "28px", maxWidth: "420px" }}>
        ...
        <p style={{ color: "#94a3b8", fontSize: "0.88rem", marginBottom: "18px", lineHeight: "1.6" }}>Adding and managing events is restricted to club coordinators.</p>
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "18px", flexWrap: "wrap" }}>
        ...
        <span style={{ background: "rgba(47,141,70,0.15)", color: "#2f8d46", padding: "3px 12px", borderRadius: "999px", fontSize: "0.76rem", fontWeight: "700", display: "flex", alignItems: "center", gap: "4px" }}><UnlockIcon /> Coordinator</span>
        <button onClick={() => setUnlocked(false)} style={{ marginLeft: "auto", background: "none", border: "1px solid #ddd", borderRadius: "6px", padding: "5px 12px", cursor: "pointer", fontSize: "0.82rem", color: "#64748b" }}>Lock</button>
      </div>
      <div style={{ background: "#172032", border: "1px solid #2d3f55", borderRadius: "12px", padding: isMobile ? "18px 14px" : "28px", maxWidth: "620px" }}>
        ...
        {addSuccess && <div style={{ background: "rgba(47,141,70,0.15)", color: "#166534", padding: "12px 14px", borderRadius: "8px", fontWeight: "600", marginTop: "12px", border: "1px solid #bbf7d0", display: "flex", alignItems: "center", gap: "8px" }}><CheckIcon /> {addSuccess}</div>}
      </div>
    </div>
  )
}
...
function Events() {
  ...
  const S = {
    ...
    tab:         { padding: isMobile ? "7px 12px" : "9px 18px", borderRadius: "8px", border: "1px solid #ddd", background: "#1e293b", cursor: "pointer", fontWeight: "600", fontSize: isMobile ? "0.8rem" : "0.88rem", color: "#94a3b8" },
    ...
    filterBtn:   { padding: "6px 14px", borderRadius: "999px", border: "1px solid #ddd", background: "#1e293b", cursor: "pointer", fontSize: "0.83rem", fontWeight: "600", color: "#94a3b8" },
    ...
    modal:       { background: "#1e293b", padding: isMobile ? "22px 18px" : "32px", borderRadius: "14px", width: "100%", maxWidth: "460px", position: "relative", boxShadow: "0 8px 32px rgba(0,0,0,0.18)", maxHeight: "90vh", overflowY: "auto" },
  }
  ...
  return (
    <div style={S.page}>
      ...
      {regSuccess && (
        <div style={{ background: "rgba(47,141,70,0.15)", color: "#166534", padding: "12px 18px", borderRadius: "10px", fontWeight: "600", marginBottom: "20px", border: "1px solid #bbf7d0", display: "flex", alignItems: "center", gap: "8px" }}>
      ...
      {(tab === "upcoming" || tab === "past") && (
        <div style={S.filterRow}>
          <span style={{ fontSize: "0.83rem", color: "#64748b", marginRight: "2px" }}>Filter:</span>
      ...
      {tab === "upcoming" && (
        <>
          ...
          {upcomingEvents.length === 0
            ? <p style={{ color: "#475569" }}>No upcoming events in this category.</p>
      ...
      {tab === "past" && (
        <>
          ...
          {pastEvents.length === 0
            ? <p style={{ color: "#475569" }}>No past events in this category.</p>
            ...
            return (
              <div key={a.id} style={{ display: "flex", gap: "12px", padding: isMobile ? "14px" : "16px 20px", borderRadius: "10px", marginBottom: "10px", background: st.bg, border: st.border, alignItems: "flex-start" }}>
                ...
                <div>
                  ...
                  <div style={{ color: "#94a3b8", fontSize: "0.87rem", lineHeight: "1.6" }}>{a.body}</div>
      ...
      {tab === "registrations" && (
        <>
          ...
          {registrations.length === 0
            ? <p style={{ color: "#475569" }}>No registrations yet.</p>
            : registrations.map(r => (
                <div key={r.id} style={{ border: "1px solid #2d3f55", borderRadius: "10px", padding: isMobile ? "12px 14px" : "14px 18px", marginBottom: "10px", background: "#1e293b", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "8px" }}>
                  <div>
                    <div style={{ fontWeight: "700", fontSize: "0.93rem" }}>{r.name}</div>
                    <div style={{ color: "#64748b", fontSize: "0.8rem" }}>{r.email} · {r.regNumber} · {r.classSection}</div>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "3px" }}>
                    <span style={{ background: "rgba(47,141,70,0.15)", color: "#2f8d46", padding: "3px 10px", borderRadius: "999px", fontSize: "0.77rem", fontWeight: "600" }}>{r.event}</span>
                    <span style={{ color: "#475569", fontSize: "0.73rem" }}>{r.time}</span>
                  </div>
                </div>
              ))
          }
      ...
      {showModal && selectedEvent && (
        <div style={S.overlay} onClick={e => e.target === e.currentTarget && setShowModal(false)}>
          <div style={S.modal}>
            <button onClick={() => setShowModal(false)} style={{ position: "absolute", top: "12px", right: "12px", background: "#263348", color: "#cbd5e1", border: "none", borderRadius: "50%", width: "28px", height: "28px", cursor: "pointer", fontWeight: "700", fontSize: "0.95rem" }}>×</button>
            <div style={{ fontWeight: "800", fontSize: isMobile ? "1rem" : "1.2rem", marginBottom: "4px" }}>Register — {selectedEvent.title}</div>
            <div style={{ color: "#64748b", fontSize: "0.86rem", marginBottom: "18px" }}>
